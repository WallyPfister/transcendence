...
export class ChannelGateway {
  ...
  async changePassword(
    ...
    if (roomId === undefined || roomId === '') {
      socket.emit(
        'errorMessage',
        'You cannot change a room password for a room with empty name.',
      );
      return;
    }
    const existingRoom = Object.values(this.chatRoomList).find(
    ...
    if (existingRoom === undefined) {
      ...
      socket.emit(
        'errorMessage',
        `You are not the chief of the room ${roomId}.`,
      );
      return;
    }
    ...
    socket.emit(
      'systemMessage',
      `You have changed the password of the room ${roomId}.`,
    );
  }
  ...
  async joinPrivateRoom(
    ...
    if (Room === undefined) {
      socket.emit('errorMessage', 'The room does not exist.');
      return;
    } else if (Room.password !== data.password) {
      socket.emit('errorMessage', 'You used a wrong password.');
      return;
    } else if (Room.banList.includes(socket.data.nickname) === true) {
